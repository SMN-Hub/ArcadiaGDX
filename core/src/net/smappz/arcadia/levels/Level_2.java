...
public class Level_2 extends Level {
    ...
    private void createEnemies() {
        final Route fromTopLeft = new Route();
        fromTopLeft.addPoint(-30, 1200).addPoint(750, 0);

        final Route fromTopRight = fromTopLeft.clone();
        RouteDriver.reverse(fromTopRight, WIDTH);

        final Route fromLeft = new Route();
        fromLeft.addPoint(-30, 800).addPoint(WIDTH/2, 200).addPoint(WIDTH+30, 800);

        final Route fromRight = fromLeft.clone();
        RouteDriver.reverse(fromRight, WIDTH);
        ...
        TimeEvent wave1 = new SquadronWave() {
            ...
            public AirSquadron init() {
                return new AirSquadron(army, fromTopLeft, Formation.Vertical, 1, 1, 3, 1);
            }
        };
        TimeEvent wave2 = new SquadronWave() {
            @Override
            public AirSquadron init() {
                return new AirSquadron(army, fromTopRight, Formation.Vertical, 1, 1, 3, 1);
            }
        };
        ...
        TimeEvent waveLoopLeft = new SquadronWave() {
            @Override
            public AirSquadron init() {
                return new AirSquadron(army, fromLeft, Formation.Vertical, 1, 1, 3, 4, 1, 1);
            }
        };
        TimeEvent waveLoopRight = new SquadronWave() {
            @Override
            public AirSquadron init() {
                return new AirSquadron(army, fromRight, Formation.Vertical, 1, 1, 3, 4, 1, 1);
            }
        };
